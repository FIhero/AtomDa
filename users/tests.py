...
class UserModelTest(TestCase):
    ...
    def test_user_email_unique(self):
        ...
        User.objects.create_user(email="duplicate@example.com", password="testpass123")

        with self.assertRaises(Exception):
            User.objects.create_user(
                email="duplicate@example.com", password="anotherpass123"
            )

    def test_user_optional_fields(self):
        """Тест необязательных полей пользователя"""
        user = User.objects.create_user(
            email="optional@example.com", password="testpass123"
        )
